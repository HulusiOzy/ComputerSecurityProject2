...
def est_extract():
    ...
    def est_bit_proportion(data):
        #Returns sum of every 1 in data
        #Also divides it by 8
        return sum(bin(byte).count('1') for byte in data) / (len(data) * 8)
    
    for p in range(1, 4):
        for q in range(1, 4):
            #Wanted to redo these so I dont have the implement them all over again
            #Reusing teachers code seemed like a better option
            ...
            #bp = proportion
            proportion = est_bit_proportion(message_byte_array)
    ...
    message_bit_array = est_extract_bits_from_image(image_byte_array, header_len, correct_p, correct_q)
    message_byte_array = est_convert_bits_to_bytes(message_bit_array)
